...
pip install kafka-python influxdb-client scikit-learn numpy

#### สเต็ปที่ 2: บันทึกโค้ดและรันโปรแกรมประมวลผล AI
1. บันทึกโค้ดใน Canvas ด้านบนเป็นไฟล์ชื่อ `realtime_ai_worker.py` ในเครื่องของอาจารย์
2. รันสคริปต์ประมวลผลด้วยคำสั่ง:
   ```bash
   python realtime_ai_worker.py
   3. สังเกตหน้าจอคอนโซล สคริปต์จะเริ่มทำการสแกนหาข้อความใหม่จากระบบคลาวด์ Kafka โดยตรง และประมวลผลสมการฟิสิกส์ร่วมกับ AI โมเดล จากนั้นบันทึกลงสู่ Measurement ชื่อ `sensor_analytics` บน InfluxDB อัตโนมัติ

#### สเต็ปที่ 3: ตรวจสอบและพล็อตกราฟบน Grafana
เพื่อตรวจสอบว่าข้อมูลทั้งหมดรวมถึงค่า Soft Sensors & AI ได้วิ่งเข้ามานอนใน InfluxDB สำเร็จแล้วจริง ๆ หรือไม่ ให้อาจารย์สร้าง Panel ใหม่ใน Grafana และรันคำสั่งสืบค้น Flux ตัวนี้ดูครับ:

```javascript
from(bucket: "iot_data")
  |> range(start: v.timeRangeStart, stop: v.timeRangeStop)
  |> filter(fn: (r) => r["_measurement"] == "sensor_analytics")
  |> filter(fn: (r) => r["id"] == "ID_68123456789") // ใส่ไอดีเซนเซอร์ทดสอบ
  // ดึงค่าฟิลด์ที่เราคำนวณและประมวลผลจากสคริปต์ AI
  |> filter(fn: (r) => r["_field"] == "temperature" or r["_field"] == "dew_point" or r["_field"] == "vpd" or r["_field"] == "altitude" or r["_field"] == "ai_anomaly")
  |> aggregateWindow(every: v.windowPeriod, fn: mean, createEmpty: false)
  |> yield(name: "mean")

เมื่อสตรีมข้อมูลเซนเซอร์ทำงานรันคู่กัน อาจารย์และนักศึกษาจะพบส้นกราฟสุดอัจฉริยะ ทั้งเส้นประเมินหยดน้ำ, ค่าต่างแรงดันไอของพืช, ความสูงสัมพัทธ์ของอาคาร และแถบสถานะการตรวจจับผิดปกติของ AI พล็อตพาดขนานเคียงคู่อุณหภูมิจริง แสดงผลแบบเรียลไทม์สวยงามทันทีครับ!
...
for message in consumer:
    try:
        ...
        if temp is not None and sensor_id is not None:
            ...
            print(f"\n[STREAM] {sensor_id} -> มีข้อมูลสะสมในบัฟเฟอร์แล้ว {current_slide_len}/{WINDOW_SIZE} จุด")
...
## 3. วิธีการพล็อตกราฟพยากรณ์ล่วงหน้าคู่กับค่าจริงบน Grafana

จุดเด่นของการบันทึกเวลาหลัก (Timestamp) ของข้อมูลทำนายเป็น "เวลาอนาคต" ทำให้บนแดชบอร์ดของ Grafana สามารถพล็อตข้อมูลเส้นทำนายยื่นล่วงหน้าพาดลอยไปรอก่อนค่าจริงได้ทันทีอย่างน่าทึ่งครับ!

### ขั้นตอนตั้งค่าในคิวรีภาษา Flux:
ให้นักเรียนสร้าง Panel ประเภท **Time Series** และระบุคิวรี Flux ดังนี้เพื่อดึงทั้งค่าอุณหภูมิจริง ณ ปัจจุบัน มาพล็อตเทียบกับค่าทำนายในอนาคต:

```javascript
from(bucket: "iot_data")
  // ตรวจสอบความถูกต้อง: ปรับช่วงเวลา Range ให้ยืดเผื่อไปข้างหน้าด้วย เช่น เผื่ออนาคตไปอีก 30 นาที (+30m)
  |> range(start: v.timeRangeStart, stop: 30m) 
  |> filter(fn: (r) => r["_measurement"] == "sensor_analytics")
  |> filter(fn: (r) => r["id"] == "ID_68123456789")
  // ดึงทั้งค่าอุณหภูมิจริงในอดีต-ปัจจุบัน และค่าทำนายในอนาคตมาวาดคู่กัน
  |> filter(fn: (r) => r["_field"] == "temperature" or r["_field"] == "temperature_predicted")
  |> aggregateWindow(every: v.windowPeriod, fn: mean, createEmpty: false)
  |> yield(name: "mean")
```

### 📌 คำแนะนำเพื่อความสวยงามในงานนำเสนอ:
* ปรับแต่งสีของเส้นในแท็บขวาของ Grafana: 
  * เส้นอุณหภูมิปัจจุบัน (`temperature`) กำหนดให้เป็น **สีส้มทึบ** 🟠
  * เส้นทำนายล่วงหน้า (`temperature_predicted`) กำหนดให้เป็น **สีน้ำเงินเป็นเส้นประ (Dashed Line)** 🔵
* นักศึกษาจะเห็นภาพอุณหภูมิจริงวิ่งไล่กวดเกลี่ยตามเส้นประทำนายที่พยากรณ์ไว้ล่วงหน้าได้อย่างชัดเจน แสดงถึงศักยภาพการทำวิศวกรรมสตรีมร่วมกับปัญญาประดิษฐ์ในระดับสากลครับ!